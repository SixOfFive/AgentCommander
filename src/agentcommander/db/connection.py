...
def _write_self_gitignore(directory: Path) -> None:
    """Drop a ``.gitignore`` inside ``directory`` ignoring everything.

    Lets a user run ac inside a tracked git project without polluting
    `git status` with `.agentcommander/` or `logs/`. No-ops if the
    directory doesn't exist (e.g. logs/ before the first message) or
    the file already exists. Best-effort — never raises.
    """
    ...
    try:
        target.write_text(
            "# Auto-generated by AgentCommander.\n"
            "# Everything in this directory is local-only state.\n"
            "*\n",
            encoding="utf-8",
        )
    except OSError:
