    ...
    cmds = [
        ...
        SlashCommand(
            ...
            details=(
                ...
                "128k = 131072 tokens.\n"
                "\n"
                "minctx <N>        drops candidates whose catalog contextLength\n"
                ...
                "                  re-run with a new minctx to re-pick them.\n"
                "                  Pre-existing /roles set overrides (no context)\n"
                "                  are preserved.\n"
                "\n"
                "memory <budget>   drops candidates whose catalog estimatedVramGb\n"
                "                  exceeds the budget. Suffixes: gb / g (default),\n"
                "                  tb / t (1024 GB). Bare numbers are GB. Models\n"
                "                  without an estimate in the catalog pass through\n"
                "                  the filter (we only filter when we can measure).\n"
                "                  Layers with minctx — combine to require both. The\n"
                "                  picks are NOT persisted on memory-only runs (it's\n"
                "                  a session filter); pair with minctx to persist.\n"
                "                  Useful when you've moved to a smaller/larger card\n"
                "                  and want autoconfig to redo its picks against the\n"
                "                  new budget without editing every role manually.\n"
                "\n"
            ...
            examples=(
                ...
                "/autoconfig memory 12gb",
                "/autoconfig memory 6.5g",
                "/autoconfig minctx 32k memory 16gb",
